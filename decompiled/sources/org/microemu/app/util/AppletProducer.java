...
/* loaded from: avacs.jar:org/microemu/app/util/AppletProducer.class */
public class AppletProducer {
    public static void createHtml(File htmlOutputFile, DeviceImpl device, String className, File midletOutputFile, File appletPackageOutputFile, File deviceOutputFile) throws IOException {
        ...
        try {
            ...
            if (deviceOutputFile != null) {
                writer.write(String.valueOf(deviceOutputFile.getName()) + ",");
            }
            writer.write(String.valueOf(midletOutputFile.getName()) + "\">\n");
    ...
    public static void createMidlet(URL midletInputUrl, File midletOutputFile) throws IOException {
        ...
        try {
            JarInputStream jis = new JarInputStream(midletInputUrl.openStream());
            ...
            while (true) {
                ...
                if (!jarEntry.isDirectory()) {
                    ...
                    while (true) {
                        int read = jis.read(inputBuffer, size, length);
                        if (read <= 0) {
                            break;
                        }
                        size += read;
                        length = 1024;
                        if (size + 1024 > inputBuffer.length) {
                            byte[] newInputBuffer = new byte[size + 1024];
            ...
            if (url != null) {
                ...
                while (true) {
                    JarEntry jarEntry2 = ijis.getNextJarEntry();
                    if (jarEntry2 == null) {
                        break;
                    }
                    if (jarEntry2.getName().equals("org/microemu/Injected.class")) {
                        jos.putNextEntry(new JarEntry(jarEntry2.getName()));
                        while (true) {
                            int read2 = ijis.read(inputBuffer);
                            if (read2 <= 0) {
                                break;
                            } else {
                                jos.write(inputBuffer, 0, read2);
                            }
                        }
                    }
                }
    ...
    public static void main(String[] args) throws ClassNotFoundException, NumberFormatException {
        ...
        try {
            ...
            if (descriptorLocation != null) {
                EmulatorContext context = new EmulatorContext() { // from class: org.microemu.app.util.AppletProducer.1
                ...
                URL[] urls = {deviceInputFile.toURI().toURL()};
